package Seminar3.homework3.controller;
...
public class UserController {
    ...
    private boolean Parse(String[] usersArray) {
        try {
            for (String data : usersArray) {
                if (data.matches("[a-zA-Zа-яА-Я]+") && !data.equals("f") &&
                        !data.equals("m")) {
                    if (data.equals(usersArray[0])) {
                        model.setSecondName(data);
    ...
    private void Save(){
        try (FileWriter writer = new FileWriter("Seminar3\\homework3\\filles\\" + model.getSecondName() + ".txt", true)) {
